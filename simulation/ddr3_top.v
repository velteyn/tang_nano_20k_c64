...
localparam [26:0] START_ADDR = 27'h0;
localparam [26:0] TOTAL_SIZE = 27'h800000;       // Test 8MB
//localparam [26:0] TOTAL_SIZE = 32*1024*1024;       // Test 64MB
...
ddr3_controller #(.ROW_WIDTH(14), .COL_WIDTH(10)) u_ddr3 (
        ...
        WRITE_BLOCK: begin
            ...
            if (addr == ADDR'(START_ADDR + TOTAL_SIZE)) begin
                ...
                if (work_counter == 0) begin
                    if (!refresh_needed) begin
                        wr <= 1'b1;
                        din <= addr[15:0] ^ {6'b0, addr[26:16]} ^ 16'd59;
                        refresh_cycle <= 0;
                    end else begin
                        refresh <= 1'b1;
                        refresh_executed <= 1'b1;
        ...
        VERIFY_BLOCK: begin
            if (addr == ADDR'(START_ADDR + TOTAL_SIZE)) begin
                ...
                if (work_counter == 0) begin
                    ...
                    // verify result
                    expected <= addr[15:0] ^ {6'b0, addr[26:16]} ^ 16'd59;
                    actual <= dout;
                    actual128 <= dout128;
                    if (dout[7:0] != BYTE'(addr ^ {6'b0, addr[26:16]} ^ 16'd59)) begin       // only test lower byte
                        error_bit <= 1'b1;
                        end_state <= state;
                        state <= FINISH;
                    end
